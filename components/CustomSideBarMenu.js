...
export default class CustomSideBarMenu extends Component{
  ...
  getUserProfile(){
    db.collection("users").where("email_id","==",this.state.userId).onSnashot((querySnapshot)=>{
    ...
    return(
      <View style={{flex:1}}>
        ...
        <Avatar rounded source={{uri:this.state.image}}size="medium"
         onPress={()=>this.selectPicture()} containerStyle={styles.imageContainer} showEditButton/>
